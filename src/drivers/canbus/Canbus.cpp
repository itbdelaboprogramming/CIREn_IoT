...
int Canbus::send() {
    ...
    if(txHeader.isExtended) {
        ...
        if (ret != ESP_OK) {
            ...
            LOGI(TAG_CAN_LIB, "CAN message sent successfully.");
        }
    ...
    // LOGI(TAG_CAN_LIB, "CAN message sent: ID = 0x%03X, DLC = %d", frame.can_id, frame.can_dlc);
    // for (int i = 0; i < frame.can_dlc; i++) {
    //     LOGI(TAG_CAN_LIB, "Data[%d] = 0x%02X", i, frame.data[i]);
    // }
...
int Canbus::requestId(uint8_t c0, uint8_t c1, uint8_t c2, uint8_t c3, uint8_t c4, uint8_t c5) {
    ...
    uint16_t crc = calculateCrc(this->macAddress, 6);

    txHeader.extendedId = crc << 3 | 5; 
    txHeader.dlc = 6; 
    txHeader.rtr = false; 
    ...
    if (mcp2515.readMessage(&frame) == MCP2515::ERROR_OK) {
        ...
        return this->deviceId; // Return the received ID
    }
...
uint16_t Canbus::calculateCrc(uint8_t* data, int length) {
...
void Canbus::setExtendedId(uint16_t deviceId, uint16_t SensorId) {
    this->txHeader.extendedId = (deviceId << 16) | SensorId;
...
void Canbus::setMessageheartbeat() {
    this->txHeader.extendedId = this->deviceId << 8 | 0; // Set standard ID for heartbeat message
